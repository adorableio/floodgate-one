...
gulp.task('stylus', function(err, cb) {
...
gulp.task('watch-pre-tasks', function(callback) {
  runSequence('clean', 'assets', ['coffee', 'stylus', 'ejs', 'jade'], callback);
});
  ...
  if (livereload) {
    findPort([35729], function(ports) {
      ...
      if (ports.length > 0) {
        ...
        console.log("[LiveReload] Can't start LiveReload => ALREADY RUNNING".red);
      }
